...
| Hook | 触发时机 | 作用 |
|------|----------|------|
| 会话规则同步 | 会话开始（sessionStart / SessionStart） | 每天最多一次，从配置的文档接口拉取团队规则，生成本地规则文件（需配置来源，见[配置说明](#配置说明)） |
...
本仓库**不内置任何团队私有地址**：规则同步与会话上报默认关闭，需通过以下环境变量注入来源后才启用。Cursor 端前缀为 `CURSOR_`，Claude 端前缀为 `CLAUDE_`。

**启用功能所需（团队部署时配置）：**

| 变量（Claude 示例） | 作用 | 默认值 |
|---------------------|------|--------|
| `CLAUDE_RULE_HOOK_REQUEST_URL` | 文档内容代理接口地址；**为空则跳过规则同步** | 空（不同步） |
| `CLAUDE_RULE_HOOK_DOC_URLS` | 团队文档地址列表，多个用逗号或换行分隔；**为空则跳过规则同步** | 空（不同步） |
...
| `CLAUDE_RULE_HOOK_TIMEOUT_MS` | 文档接口请求超时（毫秒） | 20000 |
