...
flowchart TD
    ...
    C -- "否或读取失败" --> E["先写入今日日期"]
    E --> F["遍历规则文件清单"]
    F --> G["优先从 GitHub raw 下载"]
    G --> H{"raw 成功?"}
    H -- "是" --> J["写入规则文件"]
    H -- "否且来源为 GitHub" --> M["改用 jsDelivr CDN 镜像重试"]
    M --> N{"jsDelivr 成功?"}
    N -- "是" --> J
    N -- "否" --> I["记录错误，跳过该文件不阻塞启动"]
    H -- "否且来源非 GitHub" --> I
...
规则文件已随本仓库发布，只维护**一套** `.md`（`rules/` 目录）。会话开始时每天最多一次从仓库 raw 地址下载到本地：Claude 端原样写为 `.md`，Cursor 端落地时改写扩展名为 `.mdc`（正文相同）。

**下载兜底**：优先从 GitHub raw（`raw.githubusercontent.com`）下载；若失败（国内直连常因 DNS 污染中断），自动改用 jsDelivr CDN 镜像（`cdn.jsdelivr.net/gh/...`，Fastly 加速）重试一次。兜底地址由 `BASE_URL` 自动推导，仅当来源是 GitHub raw 时启用；若把 `BASE_URL` 覆盖为内网或后端地址，则不推导、不兜底。注意 jsDelivr 对分支有约 12 小时缓存，故 raw 优先以保证拿到最新内容。
