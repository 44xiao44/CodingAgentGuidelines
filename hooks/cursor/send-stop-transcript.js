...
/**
 * [业务] 在 Cursor `stop` 事件触发时，把会话内容上报到外部接口。
 * [设计] 使用 Node 内置能力（无三方依赖）；所有失败只写 stderr，默认 fail-open，不阻塞 Cursor。
 */
...
// [设计] 团队部署时通过环境变量 CURSOR_STOP_WEBHOOK_URL 注入真实上报地址后才启用上报。
const DEFAULT_ENDPOINT = "";

// [字段] ENDPOINT：实际上报地址，优先使用环境变量 CURSOR_STOP_WEBHOOK_URL。
const ENDPOINT = process.env.CURSOR_STOP_WEBHOOK_URL || DEFAULT_ENDPOINT;

// [字段] TOKEN：可选 Bearer Token；配置后会写入 Authorization 请求头。
const TOKEN = process.env.CURSOR_STOP_WEBHOOK_TOKEN || "";

// [字段] TIMEOUT_MS：HTTP 上报超时时间，避免 stop hook 长时间挂起。
const TIMEOUT_MS = Number.parseInt(process.env.CURSOR_STOP_WEBHOOK_TIMEOUT_MS || "15000", 10);

// [字段] MAX_TRANSCRIPT_BYTES：会话 transcript 最大读取字节数，默认 2 MiB。
const MAX_TRANSCRIPT_BYTES = Number.parseInt(
  process.env.CURSOR_STOP_MAX_TRANSCRIPT_BYTES || "2097152",
  10
);
// [业务] 这里要和 capture-post-tool-use.js 使用同一个目录，才能在 stop 阶段读到工具调用记录。
// [设计] 用户级 hook 从 ~/.cursor 运行，因此默认目录为 ~/.cursor/hooks/.data。
...
const MAX_TOOL_TRACE_BYTES = Number.parseInt(
  process.env.CURSOR_STOP_MAX_TOOL_TRACE_BYTES || "3145728",
  10
);

/**
 * [业务] 读取 Cursor hook 通过 stdin 传入的 stop 事件 JSON 原文。
...
/**
 * [业务] 安全解析 Cursor hook 输入或 JSONL 中的单行工具 trace。
...
/**
 * [业务] 将 conversation_id 转成可安全落盘和回读的文件名片段。
...
function sanitizeForFileName(text) {
  // [业务] conversation_id 会作为工具 trace 文件名的一部分，需要先转换成安全文件名。
 ...
 *
 * @param {string|undefined} conversationId Cursor 会话 ID。
 * @returns {Promise<object>} 工具事件列表、读取状态、原始大小、截断状态、路径和错误信息。
 */
async function readToolTrace(conversationId) {
  ...
  if (!conversationId) {
    return {
      ...
      events: [],
      // [字段] state：missing_conversation_id 表示 stop 输入缺少 conversation_id。
      state: "missing_conversation_id",
...
async function postPayload(payload) {
  // [业务] 将会话 transcript 和工具 trace 汇总后发送到本地/自定义服务端。
  // [设计] 使用 AbortController 做超时控制；失败由 main 捕获，保持 fail-open。
  if (!ENDPOINT) {
    console.error("[stop-hook] CURSOR_STOP_WEBHOOK_URL 未配置，跳过上报。");
    return;
  }
  ...
  try {
    ...
    if (TOKEN) {
      // [字段] authorization：可选鉴权头，值来自 CURSOR_STOP_WEBHOOK_TOKEN。
 ...
 */
async function main() {
  // [字段] stdinRaw：Cursor 传入的 stop 事件原始 JSON。
  const stdinRaw = await readStdin();

  // [字段] hookInput：解析后的 stop 事件对象，包含会话 ID、transcript 路径、模型等元信息。
  ...
  const toolTraceInfo = await readToolTrace(hookInput.conversation_id);

  // [业务] payload 保留 Cursor 会话元信息、transcript 正文和工具调用明细，便于服务端统一归档。
  // [设计] 内容和状态分开存放：正文放 transcript/toolTrace，读取状态与截断信息放 meta。
  // [字段] payload：最终发送给服务端的会话归档数据。
  const payload = {
    // [字段] event：事件类型，固定标识这是一条 Cursor stop 上报。
    event: "cursor_stop",
    // [字段] sentAt：hook 发送 payload 的时间，使用 ISO 字符串便于服务端排序。
    sentAt: new Date().toISOString(),
    // [字段] cursor：Cursor 会话相关元信息。
    cursor: {
      // [字段] conversationId：Cursor 会话 ID，用于服务端聚合同一会话数据。
      conversationId: hookInput.conversation_id || null,
      // [字段] generationId：最后一轮生成 ID，用于关联本次 stop 发生在哪轮响应后。
      generationId: hookInput.generation_id || null,
      // [字段] status：Cursor stop 状态，例如正常结束或其他终止状态。
      status: hookInput.status || null,
      // [字段] loopCount：stop/subagent loop 计数，用于分析是否发生自动续跑。
      loopCount: hookInput.loop_count ?? null,
      // [字段] model：本次会话使用的模型名称。
      model: hookInput.model || null,
      // [字段] cursorVersion：当前 Cursor 版本，便于排查 hook 输入格式差异。
      cursorVersion: hookInput.cursor_version || null,
      // [字段] hookEventName：触发脚本的 hook 事件名，默认 stop。
      hookEventName: hookInput.hook_event_name || "stop",
      // [字段] workspaceRoots：当前会话关联的工作区根目录列表。
      workspaceRoots: Array.isArray(hookInput.workspace_roots) ? hookInput.workspace_roots : [],
      // [字段] userEmail：Cursor 传入的用户邮箱；没有时为 null。
      userEmail: hookInput.user_email || null,
      // [字段] transcriptPath：Cursor transcript 文件路径，便于服务端或日志回溯。
